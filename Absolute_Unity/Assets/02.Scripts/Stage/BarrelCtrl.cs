...
public class BarrelCtrl : MonoBehaviour
{
    ...
    {
        //폭발 효과 프리팹을 동적으로 생성
        GameObject effect = Instantiate(expEffect, transform.position, Quaternion.identity/*특정 회전 값 없이 생성하려는 프리팹 or 3D 모델의 원래 회전 각도로 적용*/);
        Destroy(effect, 2.0f);
        ...
        //폭발음 발생
        _audio.PlayOneShot(expSfx, 0.9f);

        //셰이크 효과 호출
        StartCoroutine(shake.ShakeCamera(0.2f, 0.3f, 0.8f));
    }
    ...
    {
        //주변에 있는 드럼통을 모두 추출
        Collider[] colls = Physics.OverlapSphere(position, expRadius, 1 << 11);
